...
## GitHub Pages = real proof (not mock)

This repo includes a live proof site in `docs/`:

- `docs/index.html` runs **real network calls** to OSV and npm registry from the browser.
- `docs/proof/latest.json` is generated by running the **actual DepGuard core code** in CI (`scripts/generate-proof.mjs`).
- No mocked or hardcoded vulnerability responses are used.
...
After deploy, your proof URL will be:
...
- `.github/workflows/proof-artifact.yml`
  - Runs real DepGuard checks and refreshes `docs/proof/latest.json`.
